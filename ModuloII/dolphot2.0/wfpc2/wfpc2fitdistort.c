...
void usage(char*exe) {
   printf("Usage: %s <CM> <<Xmax> <Ymax>> <<flags>>\n",exe);
...
int main(int argc,char**argv) {
   ...
   if (argc==1) {
	 ...
	 N=0;
	 rv=0.;
	 for (i=0;i<=800;i+=50) for (j=0;j<=800;j+=50) {
	    X=i; Y=j;
	    WFPC2fwddistort(cm,&X,&Y);
	    WFPC2revdistort(cm,&X,&Y);
	    rv+=(X-i)*(X-i)+(Y-j)*(Y-j);
	    N++;
	 }
	 printf("%d %f\n",cm,sqrt(rv/N));
   ...
   if (argc==2) {
	 ...
	 N++;
      ...
      func2data=WFPC2filters[filt].idc[1][cm];
      for (i=0;i<28;i++) p[i]=WFPC2filters[filt].idc[1][cm][5+i];
      printf("rms0 = %f\n",sqrt(func2(p-1)));
      xi=matrix(1,28,1,28);
      for (i=1;i<=28;i++) {
