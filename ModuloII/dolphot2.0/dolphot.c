...
FILE *of,*falign=NULL,*fapcor=NULL,*fpsfs=NULL,*ffakeout,*fwarn,*fverb;
ftype refpsf;
...
static inline float modelnoise(int img,int x,int y,int dx,int dy,float s,float ssky) {
   ...
   n=data[img][y][x]-res[img][y][x]; // model of other subtracted stars
   if (n<0.) n=0.;
   if (psf[dy][dx]>0. && s>0.) n+=s*psf[dy][dx]; // add own star model
   if (ssky>0.) {
      if (n<data[img][y][x]-ssky) n=data[img][y][x]; // limit to >= data[]
      else n+=ssky; // add sky
   }
   else if (n<data[img][y][x]) n=data[img][y][x]; // limit to >= data[]
   return n/iGAIN[img]+iRN[img];
}
...
void setindx(int i) {
   int j;
   if (i==0) {
      for (j=YMIN*SubResRef;j<YMAX*SubResRef;j++) memset(indx[j]+XMIN*SubResRef,0,FLOATSIZE*(XMAX-XMIN)*SubResRef);
   }
   else {
      for (j=XMIN*SubResRef;j<XMAX*SubResRef;j++) tindx[j]=i;
      for (j=YMIN*SubResRef;j<YMAX*SubResRef;j++) {
	 memcpy(indx[j]+XMIN*SubResRef,tindx+XMIN*SubResRef,FLOATSIZE*(XMAX-XMIN)*SubResRef);
      }
   }
   return;
}
...
float getsky_norm(int img,int x,int y,float*unc) {
   ...
   for (x1=x-rsky;x1<=x+rsky;x1+=SkipSky) for (y1=y-rsky;y1<=y+rsky;y1+=SkipSky) if (ppixOK(img,x1,y1)) {
      dx=abs(x-x1);
      dy=abs(y-y1);
...
float getsky_small(int img,int x,int y,float*unc) {
   ...
   if (unc) *unc=0.;
#endif
   for (y1=y-RSky21[img];y1<=y+RSky21[img];y1++) for (x1=x-RSky21[img];x1<=x+RSky21[img];x1++) if (ppixOK(img,x1,y1)) {
      r2=sqrt((x1-x)*(x1-x)+(y1-y)*(y1-y));
      if (r2>RSky20[img] && r2<=RSky21[img]) {
	 list[N++]=res[img][y1][x1];
#ifdef USESKYSIG
...
float getsky(int img,int x,int y,float*unc) {
   ...
   else if (FitSky==2) tmp=getsky_small(img,x,y,unc);
   else tmp=getsky_norm(img,x,y,unc);
   if (tmp<0 && !NegSky) return 0.0;
   else return tmp;
}
...
static inline void eval1_apphot(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*cs) {
   float c=0,d=0;
   int y1,x1,dx,dy;
...
static inline void eval1_psfphot(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*cs) {
   float c,d,w,n,s0=0.;
#ifdef USESKYSIG
   float dsky;
#endif
...
static inline void eval1_apphot_sky3(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*cs) {
   float c=0.,d=0.;
   int y1,x1,dx,dy;
...
static inline void eval1_psfphot_sky3(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*cs) {
...
static inline void eval1_apphot_sky4(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*mx,float*my,float*cs) {
   float c=0.,d=0.;
   int y1,x1,dx,dy;
...
static inline void eval1_psfphot_sky4(int img,int ix,int iy,int cx,int cy,float*s,float*ss,float*ssky,float*mx,float*my,float*cs) {
...
float eval1(int img,double x,double y,float pa,float pb,float pc,float*s,float*ss,float*c,float*ndof,float*sh,float*ssky,float*cm,int phot,int full,int star,int newpsf) {
   ...
   *c=*sh=A=np=*ndof=0;
   ...
   (*ssky)*=(*cm);
#ifdef NAN_PRINT
   if (isnan(m)) printf("Uncaught mm=nan\n");
#endif
#ifdef NAN_CRASH
   assert(!isnan(m));
#else
...
void phot(int x,int y,int mark,int nit) {
   ...
   if (sx<XMIN || sx>=XMAX || sy<YMIN || sy>=YMAX || indx[(int)(sy*SubResRef)][(int)(sx*SubResRef)]>=0) m=0;
   ...
   if ((bs>=bss*SigFind && bs>0 && m>0) || mark>=0) {
	 ...
	 if (mark<0) for (j=-(int)RCombine;j<=(int)RCombine;j++) for (i=-(int)RCombine;i<=(int)RCombine;i++) if ((int)(sy*SubResRef)+j>=YMIN*SubResRef && (int)(sy*SubResRef)+j<YMAX*SubResRef && (int)(sx*SubResRef)+i>=XMIN*SubResRef && (int)(sx*SubResRef)+i<XMAX*SubResRef && i*i+j*j<(RCombine-0.5)*(RCombine-0.5)) ran[(int)(sy*SubResRef)+j][(int)(sx*SubResRef)+i]=1;
      }
   }
   return;
}
...
int fix1psf(int img,int Np,int**plist,int it) {
   ...
   for (i=0;i<Np;i++) if (plist[i][img+1]) {
	 ...
	 ssky[i]=getsky_norm(img,(int)tx,(int)ty,NULL);
#if defined(FIXPSF_DEBUG) && defined(FIXPSF_DEBUG_VERB)
	 if (img==FIXPSF_DEBUG) printf("    %d %g %g %g %d\n",i,tx,ty,ssky[i],NegSky);
#endif
	 if (ssky[i]<0 && !NegSky) ssky[i]=0.0;
      }
      // AVPSF will be average PSF (no sky subtraction if FitSky==2)
      if ((is=stars[j].is[img]/stars[j].icm[img])>0 && stars[j].s>0 && stars[j].s>stars[j].ss*SigFind) {
   ...
   for (y=-RPSF[img];y<=RPSF[img];y++) for (x=-RPSF[img];x<=RPSF[img];x++) {
	 ...
	 if ((is=stars[ii].is[img]/stars[ii].icm[img])>0 && stars[ii].s>0 && stars[ii].s>stars[ii].ss*SigFind) {
	    ...
	    // sig[] is sigma(star brightness) / pixel noise estimate
	    if (posOK(img,xx,yy)) {
...
int clean(int it,int show) {
   ...
   for (i=1;i<Nstars;i++) if (stars[i].x>=0) {
      rad=(int)(RCeff+0.999);
      br=R2Ceff;
      for (xx=-rad;xx<=rad;xx++) if ((x3=(int)(stars[i].x*SubResRef)+xx)>=XMIN*SubResRef && x3<XMAX*SubResRef) for (yy=-rad;yy<=rad;yy++) if ((xx || yy) && (y3=(int)(stars[i].y*SubResRef)+yy)>=YMIN*SubResRef && y3<YMAX*SubResRef && (j=indx[y3][x3])>=0 && j<i && (x=fabs(stars[i].x-stars[j].x)*SubResRef)<=RCeff && (y=fabs(stars[i].y-stars[j].y)*SubResRef)<=RCeff) {
	 r=x*x+y*y;
	 if (r<br) {
	    br=r;
	    b=j;
   ...
   */
   ...
   for (y=0;y<dataim[img].Y;y++) for (x=0;x<dataim[img].X;x++) {
	 ...
	 if (tx) tmpres[y][x]=ty/tx;
      ...
      if (data[img][y][x]<iDMAX[img] && (tmpres[y][x]<=iDMIN[Nimg] || tmpres[y][x]>=FSat*iDMAX[Nimg])) data[img][y][x] = safedown(iDMIN[img]);
   }
   free_vector(p,1,n*n);
...
void clearsnmap(void) {
   int y;
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) {
      memset(snmap[y]+XMIN*SubResRef,0,FLOATSIZE*(XMAX-XMIN)*SubResRef);
      memset(snmmap[y]+XMIN*SubResRef,0,FLOATSIZE*(XMAX-XMIN)*SubResRef);
      memset(snmapflag[y]+XMIN*SubResRef,0,(XMAX-XMIN)*SubResRef);
   }
   return;
}
...
static inline float peaksn(int img,int x,int y) {
   if (x<=XMIN*SubResRef || y<=YMIN*SubResRef || x>=XMAX*SubResRef-1 || y>=YMAX*SubResRef-1) return 0.;
   if (img>=0 && (x<=0 || y<=0 || x>=dataim[img].X*SubResRef-1 || y>=dataim[img].Y*SubResRef-1)) return 0.;
...
float alphot(int img,double x0,double y0,double*x,double*y,int finding,int strictpos) {
   ...
   if (finding!=0) {
	 ...
	 if (cl>1 || s<ss*5 || s<=0) return 0;
      ...
      if (*x<XMIN || *x>=XMAX || *y<YMIN || *y>=YMAX || indx[(int)(*y*SubResRef)][(int)(*x*SubResRef)]>=0) return 0;
...
float aminfuncWCSrev(float *c) {
   ...
   for (x0=0;x0<=dataim[ALIMG].X;x0+=xstep) {
	 ...
	 fixRA(ALIMG,x,y,&x,&y,-3);      // IMG tangent to RA/DEC
...
void convertWCS2(void) {
   ...
   for (img=0;img<Nimg;img++) if (img!=refimg) {
      ...
      fixRA(refimg,refra[0],refra[1],&r,&d,3); // convert RA/DEC to REF tangent
      shift(refimg,r,d,refpix,refpix+1,3);
...
void align(int ext,int fld) {
   ...
   */
   ...
   if (Nstars==0) {
      ...
      setindx(-1);
      for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) {
	 memset(ran[y]+XMIN*SubResRef,0,(XMAX-XMIN)*SubResRef);
      }
      clearsnmap();
      setsnmap(img,XMIN,XMAX,YMIN,YMAX);
      sigmax=1.e20;
      sigmin=200.;
   ...
   for (img=0;img<Nimg;img++) if (img || Timg>Nimg) {
	 ...
	 if (it==AlignIter-1) {
	    ...
	    if (falign!=NULL) {
	       for (i=0;i<Nalign;i++) {
		  shift(img,Alist[i][0],Alist[i][1],&dx,&dy,1);
		  fprintf(falign,"%2d %d %d %7.2f %7.2f %8.3f %8.3f %8.3f %8.3f\n",img+1,ext,fld+1,Alist[i][0],Alist[i][1],dx,dy,Alist[i][2],Alist[i][3]);
	       }
	    }
#ifdef PGPLOT
...
void find(int show) {
   ...
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) memset(ran[y]+XMIN*SubResRef,0,(XMAX-XMIN)*SubResRef);
   clearsnmap();
   NO_SKY=1;
   setsnmap(-1,XMIN,XMAX,YMIN,YMAX);
   NO_SKY=0;
   ...
   for (i=0;i<SecondPass;i++) {
	 ...
	 memset(ran[y]+XMIN*SubResRef,0,(XMAX-XMIN)*SubResRef);
      ...
      for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) for (x=XMIN*SubResRef;x<XMAX*SubResRef;x++) if (indx[y][x]>=0) {
	 ran[y][x]=1;
	 if (RCombine>=1.9) {
	    if (y>YMIN*SubResRef) ran[y-1][x]=1;
	    if (y<YMAX*SubResRef-1) ran[y+1][x]=1;
	    if (x>XMIN*SubResRef) ran[y][x-1]=1;
	    if (x<XMAX*SubResRef-1) ran[y][x+1]=1;
	 }
	 if (RCombine>=2.4) {
	    if (y>YMIN*SubResRef) {
	       if (x>XMIN*SubResRef) ran[y-1][x-1]=1;
	       if (x<XMAX*SubResRef-1) ran[y-1][x+1]=1;
	    }
	    if (y<YMAX*SubResRef-1) {
	       if (x>XMIN*SubResRef) ran[y+1][x-1]=1;
	       if (x<XMAX*SubResRef-1) ran[y+1][x+1]=1;
	    }
	 }
...
void fakestar(int ext,int z,double x0,double y0,double xw,double yw,int tw,double *ct0) {
   ...
   if (XMIN2<XMIN20) XMIN2=XMIN20; // ensure solution is within original photsec
   if (XMAX2>XMAX20) XMAX2=XMAX20;
   if (YMIN2<YMIN20) YMIN2=YMIN20;
   if (YMAX2>YMAX20) YMAX2=YMAX20;
   ...
   for (img=0;img<Nimg;img++) {
	 ...
	 if (dataOK(img,ix+x1,iy+y1)) {
	    if (RandomFake) data[img][iy+y1][ix+x1]+=poiss(ct0[img]*psf[y1][x1]*iGAIN[img])/iGAIN[img]; // AEDDEBUG need to test this works
	    else data[img][iy+y1][ix+x1]+=ct0[img]*psf[y1][x1];
	 }
      }
      // ensure at least one corner is in the WCS-defined position
      anyCornerOK = 0;
      shift(img,XMIN,YMIN,&x,&y,1); if (x>-999.0) anyCornerOK = 1;
      if (!anyCornerOK) {
	 shift(img,XMIN,YMAX,&x,&y,1); if (x>-999.0) anyCornerOK = 1;
      }
      if (!anyCornerOK) {
	 shift(img,XMAX,YMIN,&x,&y,1); if (x>-999.0) anyCornerOK = 1;
      }
      if (!anyCornerOK) {
	 shift(img,XMAX,YMAX,&x,&y,1); if (x>-999.0) anyCornerOK = 1;
      }
      if (anyCornerOK) {
	 wcsminmax_save = wcsminmax_set;
	 wcsminmax_set = 0; // temporary override to determine surrounding box
...
#ifdef USEWFPC2
      if (hstmode[img].inst==WFPC2) fprintf(ffakeout,"%6.3f ",WFPC2calcmag(img,x0,y0,ct0[img],bg[img],WFPC2useCTE));
      else
#endif
#ifdef USEACS
      if (hstmode[img].inst==ACS) fprintf(ffakeout,"%6.3f ",ACScalcmag(img,x0,y0,ct0[img],bg[img],ACSuseCTE));
      else
#endif
#ifdef USEWFC3
      if (hstmode[img].inst==WFC3) fprintf(ffakeout,"%6.3f ",WFC3calcmag(img,x0,y0,ct0[img],bg[img],WFC3useCTE));
      else
#endif
#ifdef USEROMAN
...
void allocXY(void) {
   void *ptr;
   int y;
   ran=(char**)calloc((size_t)(YMAX-YMIN)*SubResRef,PTRSIZE);
   snmapflag=(char**)calloc((size_t)(YMAX-YMIN)*SubResRef,PTRSIZE);
   snmap=(float**)calloc((size_t)(YMAX-YMIN)*SubResRef,PTRSIZE);
   snmmap=(float**)calloc((size_t)(YMAX-YMIN)*SubResRef,PTRSIZE);
   indx=(int**)calloc((size_t)(YMAX-YMIN)*SubResRef,PTRSIZE);
   tindx=(int*)calloc((size_t)(XMAX-XMIN)*SubResRef,INTSIZE);
   if (!ran || !snmapflag || !snmap || !snmmap || !indx || !tindx) merr();
   ran -= YMIN*SubResRef;
   snmapflag -= YMIN*SubResRef;
   snmap -= YMIN*SubResRef;
   snmmap -= YMIN*SubResRef;
   indx -= YMIN*SubResRef;
   tindx -= XMIN*SubResRef;
   ptr=malloc((size_t)(XMAX-XMIN)*(size_t)(YMAX-YMIN)*SubResRef*SubResRef); if (!ptr) merr();
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) ran[y] = ptr + (size_t)(XMAX-XMIN)*SubResRef*(y-YMIN*SubResRef) - XMIN*SubResRef;
   ptr=malloc((size_t)(XMAX-XMIN)*(size_t)(YMAX-YMIN)*SubResRef*SubResRef); if (!ptr) merr();
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) snmapflag[y] = ptr + (size_t)(XMAX-XMIN)*SubResRef*(y-YMIN*SubResRef) - XMIN*SubResRef;
   ptr=malloc((size_t)(XMAX-XMIN)*(size_t)(YMAX-YMIN)*SubResRef*SubResRef*FLOATSIZE); if (!ptr) merr();
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) snmap[y] = ptr + (size_t)(XMAX-XMIN)*SubResRef*(y-YMIN*SubResRef)*FLOATSIZE - XMIN*SubResRef*FLOATSIZE;
   ptr=malloc((size_t)(XMAX-XMIN)*(size_t)(YMAX-YMIN)*SubResRef*SubResRef*FLOATSIZE); if (!ptr) merr();
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) snmmap[y] = ptr + (size_t)(XMAX-XMIN)*SubResRef*(y-YMIN*SubResRef)*FLOATSIZE - XMIN*SubResRef*FLOATSIZE;
   ptr=malloc((size_t)(XMAX-XMIN)*(size_t)(YMAX-YMIN)*SubResRef*SubResRef*INTSIZE); if (!ptr) merr();
   for (y=YMIN*SubResRef;y<YMAX*SubResRef;y++) indx[y] = ptr + (size_t)(XMAX-XMIN)*SubResRef*(y-YMIN*SubResRef)*INTSIZE - XMIN*SubResRef*INTSIZE;
}

void freeXY(void) {
   free(indx[YMIN*SubResRef]+XMIN*SubResRef);
   free(snmmap[YMIN*SubResRef]+XMIN*SubResRef);
   free(snmap[YMIN*SubResRef]+XMIN*SubResRef);
   free(snmapflag[YMIN*SubResRef]+XMIN*SubResRef);
   free(ran[YMIN*SubResRef]+XMIN*SubResRef);
   free(tindx+XMIN*SubResRef);
   free(indx+YMIN*SubResRef);
   free(snmmap+YMIN*SubResRef);
   free(snmap+YMIN*SubResRef);
   free(snmapflag+YMIN*SubResRef);
   free(ran+YMIN*SubResRef);
}
...
#endif
#ifdef USEWFPC2
      if (FakeStars[0]==0 && UsePhot[0]==0 && !DRIZZLE_BASE) wfpc2tweakshift();
      wfpc2initpsf();
#endif
#ifdef USEROMAN
      romaninitpsf();
...
int main(int argc,char**argv) {
   ...
   strcpy(outfn,argv[1]);
   if (FakeStars[0]) {
      if (FakeOut[0]) strcpy(str,FakeOut);
      else sprintf(str,"%s.fake",outfn);
   ...
   for (img=0;img<Timg;img++) {
	 ...
	 if (!FakeStars[0] && !UsePhot[0]) {
	    ...
	    if (UsePhot[0]) sprintf(str,"%s.%d.psf.fits",UsePhot,img+1);
	    else sprintf(str,"%s.%d.psf.fits",outfn,img+1);
