...
int apphotparam(char*var,char*val) {
   ...
   if (!strcasecmp(var,"infn")) {strcpy(infn,val); return 1;}
   if (!strcasecmp(var,"outfn")) {strcpy(outfn,val); return 1;}
   return 0;
}
...
int main(int argc,char**argv) {
   ...
   while (getnewpos(&ext,&z,&fx,&fy)) {
	 ...
	 photstar(fout,ext,fx,fy,z-1);
      ...
      else printf("ERROR: illegal extension used\n");
   }
