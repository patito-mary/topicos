...
size_t dfread(void *ptr,size_t size,size_t N,FILE *f) {
   size_t rv;
   rv=fread(ptr,size,N,f);
   endian8((char*)ptr,(N*size)/8);
   return rv;
}

size_t dfwrite(const void *ptr,size_t size,size_t N,FILE *f) {
   ...
   if (!cp) {
      printf("Memory error in dfwrite\n");
      exit(0);
   }
   memcpy(cp,ptr,tot);
   endian8(cp,tot/8);
...
void paramfile(char*fn,int(*func)(char*,char*)) {
   ...
   sprintf(str,"%s/param/%s","/Users/patito/Documents/topicos/topicos/ModuloII/dolphot2.0",fn);
   ...
   if (camera[0]) {
      sprintf(str,"%s/%s/%s","/Users/patito/Documents/topicos/topicos/ModuloII/dolphot2.0",camera,fn);
      if ((f=fopen(str,"r"))!=NULL) {
	 any=1;
	 while (fgets(str,801,f)) parseparam(str,func);
	 fclose(f);
