...
int main(int argc,char**argv) {
   ...
   if (argc!=7) {
      printf("Usage: %s <fits base> <inner radius> <outer radius> <step> <lower sigma> <upper sigma>\n",*argv);
      printf("Use negative <step> to use NxN regions and interpolation\n");
      return 1;
   }
   rin2=atoi(argv[2]);
   if (rin2<0) rin2=0;
