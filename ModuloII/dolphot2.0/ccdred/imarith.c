...
void img1arith(imtype*im1) {
   ...
   for (z=0;z<im1->Z;z++) for (y=0;y<im1->Y;y++) for (x=0;x<im1->X;x++) {
	 ...
	 if (!flip) im1->data[z][y][x]=safedown(tDMIN);
	 else im1->data[z][y][x]=safeup(tDMAX);
      }
      else if (im1->data[z][y][x]>=DMAX) {
	 if (!flip) im1->data[z][y][x]=safeup(tDMAX);
	 else im1->data[z][y][x]=safedown(tDMIN);
      }
...
void img2arith(imtype*im1,imtype*im2) {
   ...
   for (z=0;z<im1->Z;z++) for (y=0;y<im1->Y;y++) for (x=0;x<im1->X;x++) {
      if (im1->data[z][y][x]<=DMIN1 || im2->data[z][y][x]<=DMIN2) im1->data[z][y][x]=safedown(tDMIN);
      else if (im1->data[z][y][x]>=DMAX1 || im2->data[z][y][x]>=DMAX2) im1->data[z][y][x]=safeup(tDMAX);
